<?php 
  ...
  $objq_checkdebt = mysqli_query($conn,$sql_checkdebt);
    if ($objq_checkdebt->num_rows > 0) {
        ...
        $money_debt2 = $objr_checkdebt['money_debt'];
        $total_debt = $money_debt + $money_debt2;
        $update_debt = "UPDATE salary_debt SET money_debt = $total_debt WHERE id_member = $id_member AND id_debt_list = $id_debt_list";
        mysqli_query($conn,$update_debt);
