<?php
  ...
  class PDF extends FPDF
    ...
    // Page header
      function Header()
      {
          ...
          // Title
          $this->SetTextColor(0,0,0);
          $this->Cell(0,5, iconv( 'UTF-8','cp874' ,'ข้อมูลใช้เงิน'), 0 , 1,'C');
          $this->Ln(3);
          $this->Cell(0,5, iconv('UTF-8','cp874' ,$name_member), 0 , 1,'C');
          $this->Ln(3);
          ...
          while($value_reservename = $objq_reservelist->fetch_assoc()){
            $this->Cell(30,8,iconv('UTF-8','cp874',$value_reservename['name_list']),1,0,'C');
           }
          $this->Cell(30,8,iconv('UTF-8','cp874','รวมเงิน'),1,0,'C');
          $this->Ln(8);
      }
    }
  ...
  $pdf=new PDF('P','mm','A4');
      ...
      while(strtotime($aday) <= strtotime($bday)) { 
        $pdf->Cell(30,8,iconv('UTF-8','cp874',Datethai($aday)),1,0,'C');
        $sum_money = 0;
      ...
      $pdf->Cell(30,8,iconv('UTF-8','cp874','รวม'),1,0,'C');
      $aday = $_POST['aday'];
      $bday = $_POST['bday'];
      $objq_reservelist3 = mysqli_query($conn,$sql_resevelist);
        while($value_list = $objq_reservelist3->fetch_assoc()){
          ...
          $pdf->Cell(30,8,iconv('UTF-8','cp874',$objr_history['SUM(money)']),1,0,'C');
        } 
        $pdf->Cell(30,8,iconv('UTF-8','cp874',$total_money),1,0,'C');  
  $pdf->Output();
?>