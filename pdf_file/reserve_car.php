<?php
  ...
  class PDF extends FPDF
    ...
    // Page header
      function Header()
      {
          ...
          // Title
          $this->SetTextColor(0,0,0);
          $this->Cell(0,5, iconv( 'UTF-8','cp874' ,'โอนจ่ายรายวัน'), 0 , 1,'C');
          $this->Ln(3);
          $this->Cell(0,5, iconv('UTF-8','cp874' ,'ทีมงาน : '.$name_member), 0 , 1,'C');
          $this->Ln(3);
          $this->SetFont('angsana','',15);
          $this->Cell(30,7,iconv('UTF-8','cp874','วันที่'),1,0,'C');
          $sql_resevelist = "SELECT id_list,name_list FROM reserve_list WHERE status = 4";
          $objq_reservelist = mysqli_query($conn,$sql_resevelist);
          while($value_reservename = $objq_reservelist->fetch_assoc()){
            $this->Cell(30,7,iconv('UTF-8','cp874',$value_reservename['name_list']),1,0,'C');
           }
          $this->Cell(30,7,iconv('UTF-8','cp874','รวมเงิน'),1,0,'C');
          $this->Ln(7);
      }
    }
  ...
  $pdf=new PDF('P','mm','A4');
      ...
      while(strtotime($aday) <= strtotime($bday)) { 
        $pdf->Cell(30,7,iconv('UTF-8','cp874',Datethai($aday)),1,0,'C');
        $sum_money = 0;
        ...
        while($value_reservelist  = $objq_reservelist2->fetch_assoc()){
          ...
          while($value_history = $objq_history->fetch_assoc()){
            $pdf->Cell(30,7,iconv('UTF-8','cp874',$value_history['SUM(money)']),1,0,'C');
            $sum_money = $sum_money + $value_history['SUM(money)'];
          }
        } 
        $total_money = $total_money + $sum_money;
        $pdf->Cell(30,7,iconv('UTF-8','cp874',$sum_money),1,0,'C');
        $pdf->Ln(7);
        $aday = date ("Y-m-d", strtotime("+1 day", strtotime($aday)));
      } 

      $pdf->Cell(30,7,iconv('UTF-8','cp874','รวมเงิน'),1,0,'C');
      $aday = $_POST['aday'];
      $bday = $_POST['bday'];
      $objq_reservelist3 = mysqli_query($conn,$sql_resevelist);
        while($value_list = $objq_reservelist3->fetch_assoc()){
          ...
          $pdf->Cell(30,7,iconv('UTF-8','cp874',$objr_history['SUM(money)']),1,0,'C');
        } 
        $pdf->Cell(30,7,iconv('UTF-8','cp874',$total_money),1,0,'C');  
  $pdf->Output();
?>